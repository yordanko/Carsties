...
export default function AuctionForm({auction}:Props) {
    //use react-hook-form for already defined actions and form states that can be applied to my form
    const {register, control, handleSubmit, setFocus, reset,
        formState:{isSubmitting, isValid}} = useForm({mode: 'onTouched'}) //validation will be performed on click out
    ...
    //Note data:FieldValues, this is what react-hook-form will hold
    ...
    <form className='flex flex-col mt-3' onSubmit={handleSubmit(onSubmit)}>
        ...
        {pathname === '/auctions/create' &&
            <>
                ...
                <div className='grid grid-cols-2 gap3'>
                    <Input label='Reverve price (enter 0 if no reserve' name='reservePrice' control={control} type='number' rules={{required:'Reserve price is required'}}/>
                    <DateInput label='Auction end date/time' name='auctionEnd'
                        dateFormat='dd MMMM yyyy h:mm a' showTimeSelect control={control} rules={{required:'Auction end date is required'}}/>
                </div>
            </>
        }
        
        <div className='flex justify-between'>
            <Button outline color='gray' >Canceled</Button>
